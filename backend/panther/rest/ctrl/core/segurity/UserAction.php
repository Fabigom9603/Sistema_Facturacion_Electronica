...
class UserAction implements IRequest
{
    ...
    public static function authenticator()
    {
        ...
        $keyTrimmed = trim(strtolower($key));  // quita espacios y pasa a minúscula
        $headersNormalized[$keyTrimmed] = $value;
        }

        if (isset($headersNormalized['authorization'])) {
            $keyAPI = $headersNormalized['authorization'];
