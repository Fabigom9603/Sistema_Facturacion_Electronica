function obtenerCities(idSelect) {
    const select = document.getElementById("selectState");
    const idState = select.value;

  const url = `http://localhost/Actividad_Unidad_4/backend/panther/rest/index.php?PATH_INFO=cities/stateId/${idState}`;
...
document.addEventListener("DOMContentLoaded", () => {
  const selectCountry = document.getElementById("selectState");
  
  selectCountry.addEventListener("change", () => {

        if (selectCountry.value === "") {
    // Vaciar listado del select de estados
    selectCity.innerHTML = '<option value="">Seleccione una opción</option>';
  } else {
    // Si seleccionó un país válido, cargar estados
    obtenerCities("selectCity");
  }
